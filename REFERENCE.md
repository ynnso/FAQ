...
**~~Not done, held for a later decision~~ — WRONG, corrected 2026-09-01:** this critique claimed splitting the highest-intent service clusters (Aerial, Matterport/360, Floor Plans, Virtual Staging) into their own dedicated URLs was "the single biggest remaining structural gap." It wasn't a gap at all — `shoot2sell.com` already has dedicated pages for all four (`/aerial-photography`, `/aerial-video`, `/matterport`, `/floor-plans`, `/commercial-virtual-staging`), each with its own real, distinct `FAQPage` schema, confirmed via live fetch. This SEO critique was apparently written without checking the live site structure. Sonny caught it by asking a plain question about the site — see `HANDOFF.md` §5 for detail. Still not done, low priority: separating the inline `<style>`/`<script>` into external cacheable files (540KB single file, no images so no real waterfall cost).
