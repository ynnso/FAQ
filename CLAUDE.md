...
## Behavior, as built
- 76 questions across 12 categories, all 75 unique ones represented in the `FAQPage` JSON-LD (76 HTML rows = 75 unique questions, since "How much do floor plans cost?" is a legitimate cross-listed duplicate shown in two categories). One section open at a time. Clicking outside, or opening another section, auto-collapses the current one. Includes a "Commercial Properties" section (added 2026-09-02) covering PORT/commercial pricing for Photo, Aerial, 360/Matterport, Floor Plans, and Video — separate from the MLS/residential content that makes up the rest of the page.
